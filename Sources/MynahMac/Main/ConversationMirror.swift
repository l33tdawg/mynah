...
// MARK: - How a message is drawn

/// The owner's own words.
///
/// A discrete object they sent, so it gets an edge and hugs the right margin;
/// Mynah's reply below is the page itself, so it does not. That asymmetry is
/// what makes the column read as a conversation rather than a log, and it is
/// the same shape whether the words arrived from the phone or were typed here.
struct OwnerMessage: View {
    let text: String
    /// How much of the column the bubble may never occupy, so even a one-word
    /// message stays visibly on its own side.
    let inset: CGFloat

    var body: some View {
        // A flexible leading spacer rather than `.frame(maxWidth:)`: a maximum
        // width *expands* to its limit, so every bubble would be the same size
        // and three words would sit in a mostly empty box.
        HStack(spacing: 0) {
            ...
            Text(text)
                ...
                .mynahBorder(r.card)
                // Who spoke is carried by which side of the column the message
                // sits on, and that is worth nothing to VoiceOver — which reads
                // the whole transcript as one undifferentiated run of sentences.
                .accessibilityLabel("You said: \(text)")
        }
    }
}

/// What Mynah said back. No border, no bubble, no duration and no tool phrases:
/// for a message that came off the phone none of that was recorded, and the two
/// halves of the transcript have to look like one conversation.
struct MynahMessage: View {
    let text: String
...
///
/// The one piece of chrome the mirrored transcript carries, and it earns its
/// place: without it the owner cannot tell which of these Mynah answered on
/// their phone and which they typed here a minute ago. Centred, small and grey,
/// in the manner of a day divider — not a pill, not a badge, and not an
/// imitation of Signal's own header.
