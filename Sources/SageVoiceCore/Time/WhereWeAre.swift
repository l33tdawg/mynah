...
public struct WhereWeAre: Codable, Equatable, Sendable {
    ...
    public func spokenForPrompt(now: Date = Date(), zone: TimeZone = .current) -> String? {
        ...
        return """
            ...
            \(place.joined(separator: ", ")), and today is \(day.string(from: now)).
            Unqualified place names are here unless the owner says otherwise, \
            and "near me" means here. Use today's date to work out any day the \
            owner names, and write the full date down when you record something.
            """
    }
